...
description: >
  ...
  "digest these PDFs", "split this PDF into images and text", "turn these exam papers /
...
LLM tokens** (MinerU is the sole transcriber, ADR-0010); your job is to understand the invoking
repo, confirm its **Layout Spec** (where Units land — CONTEXT: Layout Spec, ADR-0008) once, run a
fast batch, and verify. **Never** OCR or transcribe pages yourself by reading images — that
defeats the tool's design and is slow/costly.
...
The tool is a published Python CLI; run it straight from its git repo with **uvx** — no clone,
no fixed install path, works the same on any machine:

```sh
DIGEST="uvx --from git+https://github.com/sunfmin/digest-pdf digest"
```

Every call below is `$DIGEST <args…>` (uvx resolves + caches the tool on first use). All commands
are run from the **invoking repo's** working directory — that (via cwd) is the only directory this
skill cares about; the tool manages its own venv and model cache internally, so you never reference
those paths.
...
Using the Phase 0 `--inspect` rows: if any row has `needs_mineru: true`, make sure MinerU is
installed. Ask the tool (no cache path to stat) and install once if missing — idempotent, ~2 GB of
models via ModelScope; print a one-line notice, then continue (the layout is already confirmed):
```sh
$DIGEST --mineru-status || $DIGEST --install-mineru
```
...
A warm server keeps models loaded across the whole batch (the per-PDF cold load is the slow part).
The tool owns its own venv/config, so just background `--serve-mineru`, wait for health, and export
the URL; the tool's runner then talks to it over HTTP automatically. If it fails to come up, warn
and continue without the URL (the tool falls back to per-PDF CLI — slower but works).
```sh
PORT="${MINERU_PORT:-8765}"
$DIGEST --serve-mineru --port "$PORT" >/dev/null 2>&1 &
SERVER_PID=$!
...
```sh
$DIGEST <targets...> --concurrency "${DIGEST_CONCURRENCY:-}"
...
- The one file you may write is `<invoking-repo>/.digest/layout.yaml` (the Layout Spec bootstrap,
  Phase 0). Do not push, do not edit the tool's lockfile — this skill only *uses* the tool.
