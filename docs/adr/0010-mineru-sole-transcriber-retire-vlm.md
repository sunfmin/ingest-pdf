...
- **核心装轻**(无 mlx/torch);唯一重依赖是 MinerU 隔离 venv(`digest --install-mineru`)。page/outline/question 统一经它 → skill Phase A 依 `--inspect` 的 `needs_mineru` 一视同仁就绪,**试卷/课本安装不对称彻底消除**(本轮调查的起点)。
...
- **Follow-up(本次已全部完成)**:
  ① **图内联** —— image 块的 `image_path`(middle.json 单源)成 `![caption](page-NNNN.fig-K.ext)`,`emit` 把图按页级文件名拷到 Unit 目录,`outline.finalize` 移动页面时连带把 `page-NNNN.fig-*` 移进同一节目录(相对引用恒有效)。
  ② **`--pages` 真正限制 MinerU** —— `run_mineru(pages=…)` 把选中页切成临时 PDF 只跑那几页,再把 `pdf_info` **重映射回原始页索引**(跳过页补空块),重映射后的 middle.json 与切片的 `images/` 同目录(图路径仍可解析);`pages` 经 `plan()` 贯穿到策略。
  ③ **删掉空转 VLM 线程** —— pipeline 从 render→vlm→write 简化为 render→write 直连;`_VLM_SKIP` 与 `needs_vlm` 一并移除(`vlm` 参数仅剩 NoVLM 供顶层 provenance)。
